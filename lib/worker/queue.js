...
export async function enqueueExecutionBatch(inputs, timeoutMs) {
  if (WORKER_BACKEND === 'redis') {
    const { enqueueExecutionBatch: enqueueRedisBatch } = await import('./redisQueue.js')
    return enqueueRedisBatch(inputs, timeoutMs)
  }
  return queue.push(() => executeQueriesBatch(inputs, timeoutMs))
}

export default { enqueueExecution, enqueueExecutionBatch }
