...
const WORKER_BACKEND = process.env.WORKER_BACKEND || process.env.WORKER_BACKEND === 'redis' ? 'redis' : 'inproc'

if (WORKER_BACKEND === 'redis') {
  // Lazy import to avoid forcing redis deps when not used
  export { enqueueExecution } from './redisQueue.js'
} else {
  class JobQueue {
   ...
   */
  export function enqueueExecution(query, datasetId, timeoutMs) {
