...
if (!REDIS_URL) {
  throw new Error('WORKER_BACKEND=redis requires REDIS_URL (Upstash REST cannot be used for BullMQ).')
}
...
export async function enqueueExecutionBatch(inputs, timeoutMs = 5000) {
  const job = await queue.add('execute_batch', { inputs, timeoutMs }, {
    attempts: 2,
    removeOnComplete: true,
    removeOnFail: false,
    backoff: { type: 'exponential', delay: 500 }
